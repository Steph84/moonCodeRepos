...
function Machina.Load(pWindowWidth, pWindowHeight, pTileSize)
  ...
  tempCol, tempLin = Spawn(1, myMap)
  
  Machina.body = {col = tempCol, lin = tempLin, dir = "right"}
  Machina.power = myMap.size.w
  Machina.action = {right = true, left = true, up = true, down = true, drill = false, teleport = false}
  
  costTeleport = myMap.size.w * 0.75
  
end

function Spawn(pLevel, pMap)
  local col, lin
  local rdCol, rdLin
